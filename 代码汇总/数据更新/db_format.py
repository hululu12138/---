...
def split_file(file_name):
    ...
    sub_file=os.path.split(os.path.split(file_path)[0])[1]
    # print('sub_file: ',sub_file)
    return upper_path, filename,sub_file
...
def traversal_file(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            ...
            if file_modify_time > '2020-08-20 00:00:00':
                file_names.append(file_name)
    return file_names
...
class DbFormat:
    ...
    def get_db_format(self, file_name):
        ...
        upper_path, filename,subfile = split_file(file_name)
        # print('upper_path',upper_path)
        symbol=''.join(re.split('[^a-z]',filename))
        if symbol=='':
            symbol='-'
        for i, a_data in enumerate(data):
            if i > 0:
                a_data.insert(1, symbol)
                a_data.insert(2, upper_path)
                a_data.append(filename)

                # 如果是201909-201911,则：
                # a_data.append(filename[2:])

                format_data.append(a_data)
        file_name_db = '{}_db\\{}\\{}\\{}.csv'.format(self.root_dir, subfile,upper_path, filename)
