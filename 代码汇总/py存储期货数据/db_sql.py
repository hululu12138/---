...
def storage(root_dir_db, logger):
    """存储数据"""
    server_local = '(local)'
    database_local = 'master'
    user_local = 'sa'
    password_local = '123456'

    server_target = '172.13.0.67'
    database_target = 'HFData'
    user_target = 'sa'
    password_target = 'boting'
    table_name_target = 'HFData_DL1'
    # -----------------------------------------------
    # 准备工作
    # 父目录
    ...
    for i, file_name in enumerate(file_names):
        ...
        # 查询数据是否存在，存在则删除
        implementation = True
        sql = """SELECT time FROM {} WHERE symbol='{}' AND time>='{}' AND time<='{}'""".format(table_name_target,
                                                                                               symbol, time_min, time_max)
        ...
        if rows:
            ...
            # 删除数据
            sql = """DELETE FROM {} WHERE symbol='{}' and time>='{}' AND time<='{}'""".format(table_name_target,
                                                                                              symbol, time_min, time_max)
            db_target.query(sql)
            # 再次查询确认数据被删除
            sql = """SELECT time FROM {} WHERE symbol='{}' AND time>='{}' AND time<='{}'""".format(
                table_name_target, symbol, time_min, time_max)
            db_target.query(sql, False)
            rows = db_target.fetchall()
            if rows:
        ...
        if implementation:
            ...
            sql = """EXEC master..xp_cmdshell 'BCP "{}..{}" in "{}" -c -S"172.13.0.67" -U"sa" -P"boting" -t","'""".format(
